...
/**
 * IIFE (Immediately Invoked Function Expression) を使用して
 * グローバルスコープの汚染を防ぐ
 *
 * 理由：
 * - 関数名の衝突を防ぐ（他のライブラリとの競合回避）
 * - 予期しない上書きを防ぐ
 * - デバッグ時の依存関係を明確にする
 * - メモリ効率の向上（不要なグローバル変数の削減）
 */
(function () {
  /**
   * 'use strict' モードを有効化
  ...
  function loadTodos() {
    ...
    fetch('/todos')
      ...
      .then(todos => {
        ...
        if (todos.length === 0) {
          todoList.innerHTML =
            '<div class="text-center text-muted py-4"><i class="bi bi-inbox"></i> TODOがありません</div>';
          return;
        }
    ...
    fetch('/todos', {
      ...
      .then(result => {
        if (result.code === 200) {
          ...
          showAlert(
            'success',
            todoId.value ? 'TODOを更新しました' : 'TODOを追加しました'
          );
        } else {
          // エラー時：エラーメッセージを表示
